...
public class StatistikFragment extends Fragment {
        ...
        apiService.getStatistik().enqueue(new Callback<ApiResponse.StatistikResponse>() {
            ...
            public void onResponse(@NonNull Call<ApiResponse.StatistikResponse> call,
                ...
                if (response.isSuccessful() && response.body() != null) {
                    List<ApiResponse.StatistikItem> items = response.body().data;
                    updateStatistikUI(items);
                }
            }
            ...
            public void onFailure(@NonNull Call<ApiResponse.StatistikResponse> call,
                ...
                Toast.makeText(requireContext(),
                        "Gagal memuat statistik: " + t.getMessage(),
                        Toast.LENGTH_SHORT).show();
            }
        });

        // Load juga data bulan ini untuk ringkasan
        apiService.getTransaksiBulanIni().enqueue(
                new Callback<ApiResponse.DashboardResponse>() {
                    @Override
    ...
    private void updateStatistikUI(List<ApiResponse.StatistikItem> items) {
        ...
        // Update progress bar per kategori
        for (ApiResponse.StatistikItem item : items) {
            ...
            switch (item.kategori) {
                case "Makan":
                    progressMakan.setProgress(persen);
                    tvPctMakan.setText(label);
                    break;
                case "Transport":
                    progressTransport.setProgress(persen);
                    tvPctTransport.setText(label);
                    break;
                case "Belanja":
                    progressBelanja.setProgress(persen);
                    tvPctBelanja.setText(label);
                    break;
                case "Hiburan":
                    progressHiburan.setProgress(persen);
                    tvPctHiburan.setText(label);
                    break;
            }
        }

        // Update bar chart dengan data dummy 6 bulan
        // (untuk data real perlu endpoint tambahan per bulan)
        updateBarChart();
    }
    ...
    private void updateBarChart() {
        ...
        // TODO: ganti dengan data real dari API jika tersedia
